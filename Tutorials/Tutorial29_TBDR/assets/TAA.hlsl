...
SamplerState PrevColorTexture_sampler;
SamplerState VelocityGBuffer_sampler;
SamplerState DepthGbuffer_sampler;
...
{	
	...
	float2 UVoffset;
	float Depth = DepthGbuffer.SampleLevel(DepthGbuffer_sampler, pin.TexC, 0.0f).r;
	...
	float2 Velocity = VelocityGBuffer.SampleLevel(VelocityGBuffer_sampler, VelocityUV, 0).rg;
	...
	float3 CurColor = ColorTexture.SampleLevel(ColorTexture_sampler, pin.TexC, 0).rgb;	
	float3 PrevColor = PrevColorTexture.SampleLevel(PrevColorTexture_sampler, pin.TexC - UVoffset, 0).rgb;
	...
	for (y = -1; y <= 1; ++y)
	{
		for (x = -1; x <= 1; ++x)
		{
			...
			float3 NeighborhoodColor = ColorTexture.Sample(ColorTexture_sampler, SampleUV).rgb;
