...
SamplerState ShadowMapCubes_sampler;
...
float BlockerSearch(float3 ReceiverPos, float dx, uint ShadowMapIdx, float2 ddist_duv)
{
	...
	for(int i = 0; i < SampleCount; i++)
	{
		...
		float BlockerDepth = ShadowMaps[ShadowMapIdx].SampleLevel(ShadowMaps_sampler, SampleUV, 0).r; // Important: don't use anisotropic sampler!
...
float PCF(float3 ReceiverPos, float UVRadius, float2 ddist_duv, uint ShadowMapIdx)
{
    ...
    for (int i = 0; i < SampleCount; i++)
    {
		...
		//Visibility += ShadowMaps[ShadowMapIdx].SampleCmpLevelZero(ShadowMaps_sampler, SampleUV, ReceiverDepthBias).r;
	}
...
float VSM(float3 ReceiverPos, uint ShadowMapIdx)
{
	...
	float2 SampleValue = ShadowMaps[ShadowMapIdx].SampleLevel(ShadowMaps_sampler, SampleUV, 0).xy;
...
float CalcVisibilityOmni(float3 LightToPoint, uint ShadowMapIdx, float FarZ)
{
	...
	float ClosestDepth = ShadowMapCubes[ShadowMapIdx].SampleLevel(ShadowMapCubes_sampler, normalize(LightToPoint), 0).r;
