...
{	
	...
	float3 NormalW = NormalGbuffer.SampleLevel(NormalGbuffer_sampler, pin.TexC, 0).rgb;
	float3 NormalV = mul(NormalW, (float3x3)gView);
	NormalV = normalize(NormalV);
	
	// Get viewspace P
	float Pz = DepthGbuffer.SampleLevel(DepthGbuffer_sampler, pin.TexC, 0.0f).r;	
	...
	for(int i = 0; i < gSampleCount; ++i)
	{
		...
		float Rz = DepthGbuffer.SampleLevel(DepthGbuffer_sampler, UV, 0.0f).r;
