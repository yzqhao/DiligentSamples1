...
{
    float3 PosL    : ATTRIB0;
    float3 NormalL : ATTRIB1;
    float2 TexC    : ATTRIB3;
    float3 TangentU : ATTRIB2;
};
...
PixelOutput PS(VertexOut pin) //: SV_Target
{
	...
	{
        Out.BaseColor = BaseColorTexture.Sample(BaseColorTexture_sampler, pin.TexC);
	}
	...
	{
		float4 NormalMapSample = NormalTexture.Sample(NormalTexture_sampler, pin.TexC);
	...
	if (Width > 1)
	{
		Metallic = MetallicTexture.Sample(MetallicTexture_sampler, pin.TexC).r;
	}   
	...
	if (Width > 1)
    {
		Roughtness = RoughnessTexture.Sample(RoughnessTexture_sampler, pin.TexC).r;
	}
