...
static void SetAlphaTestMaterials(std::unordered_set<std::string>& mats)
{
    // San Miguel
...
static void SetTwoSidedMaterials(std::unordered_set<std::string>& mats)
{
    // San Miguel
    mats.insert("aglaonema_Leaf");
    mats.insert("brevipedunculata_Leaf1");
...
void setDefaultTextures(Scene* pScene, int index)
{
    ...
    // default textures
    pScene->textures[index] = (char*)malloc(strlen(DEFAULT_ALBEDO) + 1);
    strcpy(pScene->textures[index], DEFAULT_ALBEDO);

    pScene->normalMaps[index] = (char*)malloc(strlen(DEFAULT_NORMAL) + 1);
    strcpy(pScene->normalMaps[index], DEFAULT_NORMAL);

    pScene->specularMaps[index] = (char*)malloc(strlen(DEFAULT_SPEC) + 1);
...
void setTextures(Scene* pScene, int index, const char* albedo, const char* specular, const char* normal, uint32_t matFlags)
{
    ...
    // default textures
    pScene->textures[index] = (char*)malloc(strlen(albedo) + 1);
    strcpy(pScene->textures[index], albedo);

    pScene->specularMaps[index] = (char*)malloc(strlen(specular) + 1);
    strcpy(pScene->specularMaps[index], specular);

    pScene->normalMaps[index] = (char*)malloc(strlen(normal) + 1);
...
static void SetMaterials(Scene* pScene)
{
    ...
    for (uint32_t i = index; i < pScene->mIndexCnt / 3; i++)
    {
        ...
        // default textures
        pScene->textures[i] = (char*)malloc(defaultAlbedoSize * sizeof(char));
        strcpy(pScene->textures[i], DEFAULT_ALBEDO);

        pScene->normalMaps[i] = (char*)malloc(defaultNormalSize * sizeof(char));
        strcpy(pScene->normalMaps[i], DEFAULT_NORMAL);

        pScene->specularMaps[i] = (char*)malloc(defaultSpecSize * sizeof(char));
...
Scene* loadScene(const char* fileName, float scale, float offsetX, float offsetY, float offsetZ, IRenderDevice* pDevice, IDeviceContext* pImmediateContext)
{
    ...
    std::unordered_set<std::string> twoSidedMaterials;
...
void removeScene(Scene* scene)
{
    for (uint32_t i = 0; i < scene->mIndexCnt / 3; ++i)
    {
        ...
        {
            free(scene->normalMaps[i]);
            scene->normalMaps[i] = NULL;
        }

        if (scene->specularMaps[i])
        {
            free(scene->specularMaps[i]);
            scene->specularMaps[i] = NULL;
        }
    }

    free(scene->textures);
    free(scene->normalMaps);
    free(scene->specularMaps);

    free(scene->materialFlags);
    free(scene);
}
...
#define makeVec3(v) (float3((v).x, (v).y, (v).z))
    ...
    mesh->clusterCompacts = (ClusterCompact*)malloc(mesh->clusterCount * sizeof(ClusterCompact));
    mesh->clusters        = (Cluster*)malloc(mesh->clusterCount * sizeof(Cluster));
    ...
    for (int i = 0; i < clusterCount; ++i)
    {
        ...
        for (int triangleIndex = 0; triangleIndex < clusterTriangleCount; ++triangleIndex)
        {
            ...
            float3 triangleNormal = cross(triangle.vtx[1] - triangle.vtx[0], triangle.vtx[2] - triangle.vtx[0]);

            if (Equals((float)triangleNormal.x, 0.0f) || Equals((float)triangleNormal.y, 0.0f) || Equals((float)triangleNormal.z, 0.0f))
        ...
        const float3 center = (aabbMin + aabbMax) / 2;
        // if the axis is 0 then we have a invalid cluster

        if (Equals((float)coneAxis.x, 0.0f) || Equals((float)coneAxis.y, 0.0f) || Equals((float)coneAxis.z, 0.0f))
            validCluster = false;
        ...
        if (validCluster)
        {
            ...
            for (int triangleIndex = 0; triangleIndex < clusterTriangleCount; ++triangleIndex)
            {
                ...
                const float3 triangleNormal = normalize(cross(triangle.vtx[1] - triangle.vtx[0], triangle.vtx[2] - triangle.vtx[0]));
        ...
        mesh->clusters[i].coneCenter      = (center + coneAxis * t);
        mesh->clusters[i].coneAxis        = (coneAxis);
        ...
        const float coneCenterToCenterDistance = length((mesh->clusters[i].coneCenter) - center);
...
{
    ownerAABB->Min.x = std::min(point.x,
                                ownerAABB->Min.x);
    ownerAABB->Min.y = std::min(point.y,
                                ownerAABB->Min.y);
    ownerAABB->Min.z = std::min(point.z,
                                ownerAABB->Min.z);

    ownerAABB->Max.x = std::max(point.x,
                                ownerAABB->Max.x);
    ownerAABB->Max.y = std::max(point.y,
                                ownerAABB->Max.y);
    ownerAABB->Max.z = std::max(point.z,
                                ownerAABB->Max.z);
}
void adjustAABB(BoundBox* ownerAABB, const BoundBox& otherAABB)
{
    ownerAABB->Min.x = std::min(otherAABB.Min.x,
                                ownerAABB->Min.x);
    ownerAABB->Min.y = std::min(otherAABB.Min.y,
                                ownerAABB->Min.y);
    ownerAABB->Min.z = std::min(otherAABB.Min.z,
                                ownerAABB->Min.z);

    ownerAABB->Max.x = std::max(otherAABB.Max.x,
                                ownerAABB->Max.x);
    ownerAABB->Max.y = std::max(otherAABB.Max.y,
                                ownerAABB->Max.y);
    ownerAABB->Max.z = std::max(otherAABB.Max.z,
                                ownerAABB->Max.z);
}
