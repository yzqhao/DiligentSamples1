...
RWTexture2D<float> g_downsampled_depth_buffer[13] : register(u0);
RWBuffer<uint> g_global_atomic : register(u13);
...
{
    return g_depth_buffer.Load(int3(index, 0));
}
float4 SpdLoad(uint2 index)
{
    // 5 -> 6 as we store a copy of the depth buffer at index 0
    return g_downsampled_depth_buffer[6].Load(index).xxxx; 
}
...
inline int2 GetDimensions(RWTexture2D<float> t)
{
    uint2 d;
    t.GetDimensions(d.x, d.y);
    return d;
...
[numthreads(32, 8, 1)]
void csmain(uint3 did : SV_DispatchThreadID, uint3 gid : SV_GroupID, uint group_index : SV_GroupIndex)
{
	int2 depth_image_size =  GetDimensions(g_depth_buffer);
    ...
    int2 image_size = GetDimensions(g_downsampled_depth_buffer[0]);
