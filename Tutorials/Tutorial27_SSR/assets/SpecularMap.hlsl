...
SamplerState srcTexture_sampler : register(s0);
RWTexture2DArray<float4> dstTexture[8]  : register(u0, space0);		// 这里实际用不了11个，对齐之前的跟签名而已
...
void csmain(int3 DTid : SV_DispatchThreadID)
{
	...
	for (int i = 0; i < SampleCount; ++i)
	{
		...
		if (NdotL > 0.0)
		{
			...
			prefilteredColor += srcTexture.SampleLevel(srcTexture_sampler, L, mipLevel) * NdotL;
