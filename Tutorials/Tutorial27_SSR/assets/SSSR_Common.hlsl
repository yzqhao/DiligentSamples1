...
float FfxSssrUnpackRoughness(FFX_SSSR_ROUGHNESS_TEXTURE_FORMAT _packed)
{
    return FFX_SSSR_ROUGHNESS_OVERRIDE;
}
#else
...
float3 LoadNormal(int2 index, Texture2D<float4> tex)
{
    return FfxSssrUnpackNormals(tex.Load(int3(index, 0)));
}

float LoadRoughness(int2 index, Texture2D<float4> tex)
{
...
half GetEdgeStoppingRoughnessWeightFP16(half roughness_p, half roughness_q, half sigma_min, half sigma_max)
{
    return 1.0 - smoothstep(sigma_min, sigma_max, abs(roughness_p - roughness_q));
}
