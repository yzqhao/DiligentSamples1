...
void TMesh::CreateBox(float width, float height, float depth, uint32 numSubdivisions)
{
	...
	for (uint32 ii = 0; ii < numSubdivisions; ++ii)
		Subdivide();
...
TVertex TMesh::MidPoint(const TVertex& v0, const TVertex& v1)
{
    ...
    float3 normal  = 0.5f * (v0.Normal + v1.Normal);
    normal         = normalize(normal);
    float3 tangent = 0.5f * (v0.TangentU + v1.TangentU);
...
void TMesh::CreateSphere(float radius, uint32 sliceCount, uint32 stackCount)
{
	...
	float phiStep   = PI_F / stackCount;
    float thetaStep = 2.0f * PI_F / sliceCount;
...
void TMesh::CreateCylinder(float bottomRadius, float topRadius, float height, uint32 sliceCount, uint32 stackCount)
{
	...
	for (uint32 i = 0; i < ringCount; ++i)
	{
		...
		for (uint32 j = 0; j <= sliceCount; ++j)
		{
			...
			// This is unit length.
			vertex.TangentU = float3(-s, 0.0f, c);

			float dr = bottomRadius - topRadius;
			float3 bitangent(dr * c, -height, dr * s);

			vertex.Normal = cross(vertex.TangentU, bitangent);
...
void TMesh::GenerateBoundingBox()
{
    for (const TVertex& Vertex : Vertices)
    {
        ...
        BoundingBox.Min = std::min(BoundingBox.Min, P);
        BoundingBox.Max = std::max(BoundingBox.Max, P);
    }
}
